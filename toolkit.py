'''
The code is the same as original https://github.com/RiqiangGao/GDP-HMM_AAPMChallenge/blob/main/toolkit.py
We added a function combine_oar_dist to generate signed distance map for OARs - however the distance map is not used in the final submission as noted in the technical report.
'''
...
def combine_oar_dist(tmp_dict, need_list, norm_oar=True, OAR_DICT=None, d_max=50, signed=True):

    """
    Build OAR representations for the dataloader.

    Returns
    -------
    comb_oar : torch.Tensor  shape [1, D, H, W]
        Signed (or unsigned) distance map of the *union* of all requested OARs.
    cat_oar  : torch.Tensor  shape [32, D, H, W]
        Same as before (multi‑channel one‑hot masks) so nothing downstream breaks.
        If you never use CatStructures=True you can skip allocating this tensor.
    """

    # 1) ----- build the union mask of requested OARs --------------------
    union_oar = torch.zeros_like(tmp_dict['img'], dtype=torch.bool)  # [1,D,H,W]
    for key in (OAR_DICT or {}).keys():
        if key not in need_list:
            continue
        if key in tmp_dict:
            union_oar |= (tmp_dict[key] > 0.5)          # binary OR

    # 2) ----- distance transform (scipy needs numpy) -------------------
    binary = union_oar.squeeze(0).cpu().numpy()        # [D,H,W] bool
    dist_out = distance_transform_edt(~binary)         # outside distance
    if signed:
        dist_in  = distance_transform_edt(binary)      # inside distance
        signed_dist = dist_out - dist_in               # outside +, inside –
    else:
        signed_dist = dist_out                         # unsigned (≥0)

    # 3) ----- normalise / clip & convert back to torch -----------------
    signed_dist = torch.from_numpy(signed_dist).float()
    signed_dist = torch.clamp(signed_dist, -d_max, d_max) / d_max  # → –1 … 1
    comb_oar = signed_dist.unsqueeze(0)                             # [1,D,H,W]

    # 4) ----- keep the old multi‑channel tensor for compatibility ------
    cat_oar = torch.zeros([32] + list(tmp_dict['img'].shape)[1:],
                          dtype=tmp_dict['img'].dtype,
                          device=tmp_dict['img'].device)
    if OAR_DICT is not None:
        for key in OAR_DICT:
            if key in tmp_dict and key in need_list:
                cat_oar[OAR_DICT[key]-1] = tmp_dict[key]
